...
class OCRNeuralNetwork:
    ...
    def train(self, training_data_array):
        for data in training_data_array:
            ...
            hiddenErrors = np.multiply(np.dot(np.asmatrix(self.theta2).T, output_errors), self.sigmoid_prime(sum1))

            # Step 4: Update weights
            self.theta1 += self.LEARNING_RATE * np.dot(np.asmatrix(hiddenErrors), np.asmatrix(data['y0']))
